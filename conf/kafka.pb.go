...
type Producer struct {
	...
	// Batch send size. 0 or 1 (default) disables batching: Produce sends
	// synchronously and returns the broker result. Values > 1 enable async
	// batching: Produce enqueues and returns immediately (fire-and-forget); the
	// batch is flushed at batch_size records or after batch_timeout, whichever
	// comes first, and flush errors are only logged/counted in metrics.
	BatchSize int32 `protobuf:"varint,5,opt,name=batch_size,json=batchSize,proto3" json:"batch_size,omitempty"`
	// Batch send waiting time (max linger before a partial batch is flushed)
